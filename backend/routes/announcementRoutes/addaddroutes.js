...
const fs = require("fs");
const PDFDocument = require("../reports_tables/Payment-Report");

router.post("/generatepaymentreport", async function (req, res, next) {
    //load cuurent time
    ...
    const PaymentReportRequest = req.body.payment;
    console.log(PaymentReportRequest)
    ...
    const table = {
        headers: ["Name on card", "Card Number","Type"],
        rows: [],
    };

    for (const Payments of PaymentReportRequest) {
        table.rows.push([
            Payments.Cardname,
            Payments.Cardnumber,
            Payments.Type,
